...
function retrieveNodeField(srcdoc, node, sref) {
	...
	res = srcdoc.evaluate("wdbe:" + sref, node, nsResolver, XPathResult.ANY_TYPE, null);
...
function exportBoxs() {
	...
	res = srcdoc.evaluate("//wdbe:src/wdbe:ListWznmQVisBoxctx/wdbe:row", srcdoc, nsResolver, XPathResult.ANY_TYPE, null);
	resnode = res.iterateNext();
	while (resnode) {
...
function handleBoxMdn(_doc, evt, jnum) {
	...
	res = srcdoc.evaluate("//wdbe:src/wdbe:ListWznmQVisBoxctx/wdbe:row[@jnum=" + jnum + "]", srcdoc, nsResolver, XPathResult.ANY_TYPE, null);
	resnode = res.iterateNext();
...
function handleBoxMup(_doc, jnum) {
	...
	// retrieve box context
	res = srcdoc.evaluate("//wdbe:src/wdbe:ListWznmQVisBoxctx/wdbe:row[@jnum=" + jnum + "]/wdbe:x", srcdoc, nsResolver, XPathResult.ANY_TYPE, null);
	resnode = res.iterateNext();
	if (resnode) {
		resnode.replaceChild(srcdoc.createTextNode(doc.x0), resnode.firstChild);

		res = srcdoc.evaluate("//wdbe:src/wdbe:ListWznmQVisBoxctx/wdbe:row[@jnum=" + jnum + "]/wdbe:y", srcdoc, nsResolver, XPathResult.ANY_TYPE, null);
		resnode = res.iterateNext();
...
function handleRowMov(event, jnum, jnumBox) {
	...
	res = srcdoc.evaluate("//wdbe:src/wdbe:ListWznmQVisRowctx/wdbe:row[@jnum=" + jnum + "]", srcdoc, nsResolver, XPathResult.ANY_TYPE, null);
	resnode = res.iterateNext();
...
function addIme(jnumBox, descend) {
	...
	// retrieve box context
	res = srcdoc.evaluate("//wdbe:src/wdbe:ListWznmQVisBoxctx/wdbe:row[@jnum=" + jnumBox + "]", srcdoc, nsResolver, XPathResult.ANY_TYPE, null);
	resnode = res.iterateNext();
